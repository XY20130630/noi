...
struct Pt {
	...
	Pt operator + (const Pt &another) const {return Pt(x + another.x, y + another.y);}
	Pt operator - (const Pt &another) const {return Pt(x - another.x, y - another.y);}
};

LD cross(const Pt &x, const Pt &y) {return x.x * y.y - x.y * y.x;}

Decimal real;
Decimal ans;

Decimal calc(int pos, int ts)
{
	if (!ts) return h[0];
	int pre = g[pos][ts];
	return (calc(pre, ts - 1) + s[pos] - s[pre]) / (pos - pre + 1);
}
...
int main()
{
	...
	int st = 0;
	FOR(i, 1, n) if (h[0] > h[i]) st = i;

	FOR(i, st + 1, n) h[i - st] = h[i];

	n -= st;
	
	if (k >= n) {
		ans = h[0];
		FOR(i, 1, n) ans = (ans + h[i]) / 2;
		printf("%s\n", ans.to_string(2 * p).c_str());
		return 0;
	}
	...
	FOR(i, 1, n) FOR(j, 0, lim) f[i][j] = -0x3f3f3f3f3f3f3f3fLL;
	FOR(i, 1, n) f[i][0] = h[0];
	...
	FOR(k, 1, lim) {
		int tot = 0;
		FOR(i, 0, k - 1) {
			if (tot <= 1) tuble[++tot] = i;
			else {
				while (tot >= 2 && cross(Pt(i, s[i] - f[i][k - 1]) - Pt(tuble[tot - 1], s[tuble[tot - 1]] - f[tuble[tot - 1]][k - 1]),
										 Pt(tuble[tot], s[tuble[tot]] - f[tuble[tot]][k - 1]) - Pt(tuble[tot - 1], s[tuble[tot - 1]] - f[tuble[tot - 1]][k - 1])) >= 0)
					tot--;
				tuble[++tot] = i;
			}
		}
		...
		FOR(i, k, n) {
			...
			while (chose < tot && (long double)(s[i] - s[tuble[chose + 1]] + f[tuble[chose + 1]][k - 1]) / (1 + i - tuble[chose + 1]) > (long double)(s[i] - s[tuble[chose]] + f[tuble[chose]][k - 1]) / (1 + i - tuble[chose])) chose++;
			while (chose > 1   && (long double)(s[i] - s[tuble[chose - 1]] + f[tuble[chose - 1]][k - 1]) / (1 + i - tuble[chose - 1]) > (long double)(s[i] - s[tuble[chose]] + f[tuble[chose]][k - 1]) / (1 + i - tuble[chose])) chose--;

			g[i][k] = tuble[chose];
			f[i][k] = (long double)(s[i] - s[tuble[chose]] + f[tuble[chose]][k - 1]) / (1 + i - tuble[chose]);
	...
	FOR(i, 0, Min(u, lim)) {
		ans = calc(u, i); FOR(j, u + 1, n) ans = (ans + h[j]) / 2;
		real = Max(real, ans);
	}

	printf("%s\n", real.to_string(2 * p).c_str());
